    ...
    <center><h1 className='text-[24px] text-yellow-500 font-bold mb-8'><u>CookBook</u></h1></center>
      ...
      {Favourite.map((item) => (
            <div key={item.id} className='justify-center group'>
              <img src={item.image_url} alt={item.name} className='w-full h-48 rounded-lg image-resize transform transition-transform duration-300 group-hover:scale-110'/>
              <div className='flex justify-center py-2 px-4 flex-col items-center'>
                ...
                <div className='flex'>
                 <button className='bg-black w-[100px] text-white rounded-md my-6 mr-4 py-[10px] text-[15px] hover:text-yellow-500' onClick={()=>handleRemove(item.id)}>
                    Remove
                </button>
